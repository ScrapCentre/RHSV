...
import {
    Loader2, MapPin, Navigation, Lock, Car, Zap,
    Building2, SlidersHorizontal, Search, AlertCircle, Store,
    ArrowRight, Star, Shield, Clock, ChevronDown, CheckCircle2, Award
} from "lucide-react"
...
export default function PersonalMarketplacePage() {
    ...
    const fetchLeads = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/personal/marketplace?${params}`)
            const data = await res.json()
            if (!res.ok) throw new Error(data.message || "Failed to fetch leads")
            setLeads(data.leads || [])
            setCcs(data.ccs || [])
            setPartnerLocation(data.rvsfLocation || null)
    ...
    // ── Direct Claim handler (no payment/Razorpay) ────────────────
    const handleClaim = async (lead: Lead) => {
        ...
        try {
            const res = await fetch("/api/personal/marketplace/claim", {
                method: "POST",
            ...
            setClaimSuccess(lead._id)
            setClaimingLeadId(null)

            // Remove claimed lead from active list
            ...
            <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div>
                    <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <h1 className="text-lg font-bold text-slate-800 tracking-tight">Market Feed</h1>
                    </div>
                    <p className="text-xs text-slate-500 font-medium">
                        Browse and claim verified vehicle leads near your facilities
                    </p>
                </div>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100">
                                ...
                                {partnerLocation && (
                                    <div className="mt-2.5 flex items-center gap-1.5 text-[10px] text-slate-500">
                                        ...
                                        <span>Center: <strong className="text-slate-700">{partnerLocation.city}, {partnerLocation.state}</strong></span>
                                    </div>
                                )}
                            </div>
                        </motion.div>
                ...
                >
                    {leads.map((lead, idx) => (
                        <LeadCard key={lead._id} lead={lead} index={idx} mode={mode}
                            onClaim={handleClaim}
                            isClaiming={claimingLeadId === lead._id}
                            isClaimSuccess={claimSuccess === lead._id}
                        />
                    ))}
                </motion.div>
            )}
...
// ─── Lead Card Component ─────────────────────────────────────────
function LeadCard({ lead, index, mode, onClaim, isClaiming, isClaimSuccess }: {
    lead: Lead;
    index: number;
            ...
            <div className="relative h-36 bg-gradient-to-br from-slate-50 to-slate-100 overflow-hidden">
                {hasPhoto ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                        <Car className="w-12 h-12 text-slate-200 opacity-40" />
                    </div>
                )}
                    ...
                    {isClaimSuccess ? (
                        ...
                        <button
                            ...
                            className={`w-full flex items-center justify-center gap-1 py-2 font-bold text-[10px] rounded-lg transition-all shadow-sm group/btn ${
                                isClaiming
                                    ? "bg-red-850 text-red-200 cursor-wait"
                        ...
                        >
                            {isClaiming ? (
                                ...
                                <>
                                    <Award className="w-3 h-3 group-hover/btn:scale-105 transition-transform" />
                                    Claim Lead
