...
export default async function PersonalChatsListPage() {
    ...
    // 2. Fetch all chat threads for this Partner
    const chatThreads = await PersonalChatThread.find({ partnerId })
        .sort({ updatedAt: -1 })
        .lean()

    // 3. Resolve customer details & vehicle info via bulk lookup of unlocked leads
    const leadIds = chatThreads.map((t: any) => t.leadId)
    const unlockedLeads = await PersonalUnlockedLead.find({
        partnerId,
        leadId: { $in: leadIds }
