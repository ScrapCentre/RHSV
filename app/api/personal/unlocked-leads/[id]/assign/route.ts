...
export async function POST(
    ...
    try {
        ...
        const { ccId } = body // ccId is the _id of PersonalCollectionCenter
        ...
        // 1. Verify Unlocked Lead belongs to this partner
        const lead = await PersonalUnlockedLead.findOne({ _id: unlockedLeadId, partnerId })
        if (!lead) {
            return NextResponse.json({ message: "Unlocked lead not found" }, { status: 404 })
        }

        // 2. Verify Collection Center belongs to this partner
        const cc = await PersonalCollectionCenter.findOne({ _id: ccId, partnerId })
        if (!cc) {
            return NextResponse.json({ message: "Collection Center not found" }, { status: 404 })
        }

        // 3. Find CC Operator
        const operator = await PersonalCCOperator.findOne({ email: cc.contactPersonEmail.toLowerCase() })
        if (!operator) {
