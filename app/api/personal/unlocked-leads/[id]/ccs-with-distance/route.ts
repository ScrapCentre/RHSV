...
export async function GET(
    ...
    try {
        ...
        const lead = await PersonalUnlockedLead.findOne({ _id: unlockedLeadId, partnerId }).lean() as any
        if (!lead) {
        ...
        // 3. Fetch all Collection Centers for this Partner
        const ccs = await PersonalCollectionCenter.find({ partnerId }).lean() as any[]
