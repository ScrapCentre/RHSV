...
export async function POST(
    ...
    try {
        ...
        const senderRole = isPartnerOwner ? "partner" : "customer"
        const senderName = session.user.name || (isPartnerOwner ? "Partner" : "Customer")
        const senderId = isPartnerOwner ? partnerId : userId
        ...
        // 5. Broadcast updates via Pusher
        try {
        ...
        console.error("[Personal Offer Action API] Error:", err)
        return NextResponse.json(
