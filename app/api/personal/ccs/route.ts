...
export async function GET(request: NextRequest) {
    try {
        ...
        const centers = await PersonalCollectionCenter.find({ partnerId }).sort({ createdAt: -1 }).lean()
...
export async function POST(request: NextRequest) {
    try {
        ...
        const existingOp = await PersonalCCOperator.findOne({ email: contactPersonEmail.toLowerCase() })
        if (existingOp) {
            return NextResponse.json({ message: "A CC operator with this email already exists." }, { status: 400 })
        }

        // Create Collection Center
        const cc = await PersonalCollectionCenter.create({
            partnerId, name, fullAddress, city, state, pincode: String(pincode),
            catchmentRadius: Number(catchmentRadius),
            contactPersonName, contactPersonPhone, contactPersonEmail,
        })

        // Create CC Operator account
        await PersonalCCOperator.create({
            ccId, partnerId,
        ...
        console.error("Personal CC Create Error:", error)
        return NextResponse.json({ message: error.message }, { status: 500 })
    }
}
