        ...
        <div className={`min-h-screen bg-slate-50 flex overflow-hidden ${plusJakartaSans.className}`}>
            ...
            >
                ...
                >
                    ...
                    {/* Manual Lead Entry */}
                    <motion.div variants={sidebarLinkVariants}>
                        <Link href="/admin/manual-lead" className={`flex items-center px-4 py-2.5 rounded-xl transition-all duration-200 group relative overflow-hidden ${isActive('/admin/manual-lead') ? 'bg-[#E31E24]/10 text-[#E31E24] shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-[#E31E24]'}`}>
                            {isActive('/admin/manual-lead') && (
                                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-[#E31E24] rounded-r-full" />
                            )}
                            <PlusCircle className={`w-4 h-4 mr-3 transition-colors ${isActive('/admin/manual-lead') ? 'text-[#E31E24]' : 'text-slate-500 group-hover:text-[#E31E24]'}`} />
