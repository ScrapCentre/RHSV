...
import { ShoppingCart, Car, User, MapPin, Calendar, ChevronLeft, CheckCircle, Trash2, Phone, Mail, DollarSign, MessageCircle, Send } from "lucide-react"
...
export default function BuyDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleStatusUpdate = async (newStatus: string) => {
        ...
        try {
            const res = await apiFetch("/api/admin/requests/approve", {
                ...
                body: JSON.stringify({ id: id, type: "buy", status: newStatus })
            })
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await apiFetch(`/api/admin/requests/delete?id=${id}&type=buy`, {
                method: "DELETE"
            })
