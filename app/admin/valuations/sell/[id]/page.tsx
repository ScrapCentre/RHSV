...
import { ShoppingCart, Car, User, MapPin, Calendar, ChevronLeft, CheckCircle, Trash2, Phone, Hash, DollarSign, Building2, MessageCircle, Image as ImageIcon, Download, Send } from "lucide-react"
...
export default function SellDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchRequest = async () => {
        try {
            const res = await fetch(`/api/admin/valuations/sell/${id}`)
            if (res.ok) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                router.push("/admin/valuations/sell")
            } else {
                toast({
                    title: "Error",
    ...
    function getStatusBadge(status: string) {
        switch (status) {
            ...
            case "approved":
                return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-200"><CheckCircle className="w-3.5 h-3.5" />Approved to CC</span>
            case "approved_to_rvsf":
                return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-200"><CheckCircle className="w-3.5 h-3.5" />Approved to RVSF's</span>
            default:
