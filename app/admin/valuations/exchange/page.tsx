                            ...
                            <div key={req._id} className="bg-white dark:bg-[#0E192D] rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all active:scale-[0.98]">
                                <div className="p-4 space-y-4">
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-purple-50 dark:bg-purple-900/20 flex items-center justify-center text-purple-600 dark:text-purple-400 font-bold text-sm">
                                                {(req.customerName || "U")[0]}
                                            </div>
                                            <div>
                                                <p className="font-bold text-gray-900 dark:text-white leading-tight">{req.customerName}</p>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm py-3 border-y border-gray-50 dark:border-slate-800/50">
                                        <div>
                                            <p className="text-[10px] text-gray-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">Old Vehicle</p>
                                            <p className="font-semibold text-gray-900 dark:text-white truncate">{req.oldVehicleBrand}</p>
                                            <p className="text-[11px] text-gray-500 dark:text-slate-400 truncate">{req.oldVehicleModel} ({req.oldVehicleYear})</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-gray-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">New Choice</p>
                                            <p className="font-semibold text-gray-900 dark:text-white truncate">{req.newVehicleBrand}</p>
                                            <p className="text-[11px] text-gray-500 dark:text-slate-400 truncate">{req.newVehicleModel || "Any Model"}</p>
                                        </div>
                                    </div>

                                    <div className="flex items-center justify-between pt-1">
                                        <div className="flex flex-col">
                                            <p className="text-[10px] text-gray-400 dark:text-slate-500 font-bold uppercase tracking-wider">Contact & Reg</p>
                                            <p className="text-[11px] font-mono text-gray-600 dark:text-slate-400">{req.customerPhone}</p>
                                            <p className="text-[11px] font-mono text-gray-500 dark:text-slate-500 tracking-tighter">{req.oldVehicleRegistration}</p>
                                        </div>
                                        <Link
                                            href={`/admin/valuations/exchange/${req._id}`}
                                            className="inline-flex items-center justify-center px-4 py-2 bg-purple-600 hover:bg-purple-700 dark:bg-purple-500/10 dark:hover:bg-purple-500/20 text-white dark:text-purple-400 rounded-lg font-bold text-xs transition-all shadow-md shadow-purple-200 dark:shadow-none"
                                        >
                                            View Details
                                        </Link>
                                    </div>
